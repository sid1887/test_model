...
.EXAMPLE
    .\test-setup.ps1 -SkipServiceTest -VerboseOutput
#>
...
function Test-Endpoint {
    ...
    for ($i = 1; $i -le $RetryCount; $i++) {
        try {
            if ($VerboseOutput) {
                Write-Info "Attempt $i of $RetryCount`: Testing $Name at $Url"
            }
            ...
            if ($i -eq $RetryCount) {
                ...
                Write-Warning "$Name not responding (attempt $i of $RetryCount), retrying in $RetryDelaySeconds seconds..."
