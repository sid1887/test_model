...
class CumpairTester:
    ...
    def test_server_health(self) -> TestResult:
        ...
        try:
            # Try the docs endpoint to confirm server is running
    ...
    def run_product_list_tests(self, sample_size: int = 20) -> None:
        ...
        for i, product in enumerate(test_products, 1):
            ...
            if result.success:
                logger.info(f"[SUCCESS] Text search - {result.results_count} results, "
    ...
    def run_image_tests(self, sample_size: int = 10) -> None:
        ...
        for i, image_path in enumerate(test_images, 1):
            ...
            if result.success:
                logger.info(f"[SUCCESS] Image search - {result.results_count} results, "
                          f"top similarity: {result.top_similarity:.3f}, "
                          f"duplicates: {result.has_duplicates}")
            else:
                logger.error(f"[FAIL] Image search failed: {result.error}")
    ...
    def generate_report(self) -> None:
        ...
        if failed_results:
            ...
            for result in failed_results:
                logger.error(f"[FAIL] {result.test_type}: {result.product_name} - {result.error}")
...
def main():
    ...
    if not health_result.success:
        logger.error("[FAIL] Server health check failed, aborting tests")
        logger.error(f"Error: {health_result.error}")
        return
    
    logger.info("[SUCCESS] Server is healthy, proceeding with tests")
