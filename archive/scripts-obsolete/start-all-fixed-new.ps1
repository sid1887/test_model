...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -Full" -NoNewline; Write-Host "        Start all services (PostgreSQL, Redis, Scraper, Celery, FastAPI)"
    Write-Host "  -Minimal" -NoNewline; Write-Host "     Start core services only (PostgreSQL, Redis, FastAPI)"
    Write-Host "  -Stop" -NoNewline; Write-Host "        Stop all running services"
    Write-Host "  -Status" -NoNewline; Write-Host "      Show current service status"
    Write-Host "  -Help" -NoNewline; Write-Host "        Show this help message"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  .\start-all.ps1 -Full" -NoNewline; Write-Host "      # Start everything (recommended)"
    Write-Host "  .\start-all.ps1 -Minimal" -NoNewline; Write-Host "   # Quick start"
    Write-Host "  .\start-all.ps1 -Status" -NoNewline; Write-Host "    # Check what's running"
    Write-Host "  .\start-all.ps1 -Stop" -NoNewline; Write-Host "      # Stop all services"
    Write-Host ""
}
...
function Main {
    ...
    if ($Minimal) {
        ...
        $statusCmd = ".\start-all.ps1 -Status"
        $stopCmd = ".\start-all.ps1 -Stop"
        Write-Info "Use '$statusCmd' to check service status."
        Write-Info "Use '$stopCmd' to stop all services."
    }
}
