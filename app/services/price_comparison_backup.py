...
class CumpairPriceEngine:
    ...
    def __init__(self):
        ...
        # Enhanced e-commerce site configurations
        self.ecommerce_sites = {
            'amazon.com': {
                'selectors': {
                    ...
                    'image': ['.imgTagWrapper img', '#landingImage']
                },
                'rate_limit': 2.0,
                'search_urls': lambda query: [
                    f"https://www.amazon.com/s?k={query}",
                    f"https://www.amazon.com/s?k={query}&ref=sr_pg_2"
                ]
            },
            'ebay.com': {
                'selectors': {
                    ...
                    'image': ['.ux-image-carousel-item img', '.vi-image-gallery-img']
                },
                'rate_limit': 1.5,
                'search_urls': lambda query: [
                    f"https://www.ebay.com/sch/i.html?_nkw={query}",
                    f"https://www.ebay.com/sch/i.html?_nkw={query}&_pgn=2"
                ]
            },
            'walmart.com': {
                'selectors': {
                    ...
                    'image': ['.prod-hero-image img', '.slider-list img']
                },
                'rate_limit': 2.5,
                'search_urls': lambda query: [
                    f"https://www.walmart.com/search/?query={query}",
                    f"https://www.walmart.com/search/?page=2&query={query}"
                ]
            },
            'bestbuy.com': {
                'selectors': {
                    'title': ['.sku-title h1', '.v-fw-regular'],
                    'price': ['.priceView-hero-price span', '.priceView-customer-price span'],
                    'rating': ['.ugc-ratings-reviews', '.c-ratings-reviews-v2'],
                ...
                'search_urls': lambda query: [
                    ...
                    f"https://www.bestbuy.com/site/searchpage.jsp?st={query}&page=2"
                ]
            }
        }
    ...
    def _generate_search_urls(self, product_query: str) -> Dict[str, List[str]]:
        ...
        for site, config in self.ecommerce_sites.items():
            try:
                search_urls[site] = config['search_urls'](encoded_query)
            except Exception as e:
                logger.warning(f"URL generation failed for {site}: {e}")
