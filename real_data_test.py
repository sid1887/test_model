...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('comprehensive_product_test.log'),
...
class CumpairTester:
    ...
    def run_product_list_tests(self, sample_size: int = 20) -> None:
        ...
        for i, product in enumerate(test_products, 1):
            ...
            if result.success:
                logger.info(f"✅ Text search successful - {result.results_count} results, "
                          f"top similarity: {result.top_similarity:.3f}, "
                          f"duplicates: {result.has_duplicates}")
            else:
                logger.error(f"❌ Text search failed: {result.error}")
