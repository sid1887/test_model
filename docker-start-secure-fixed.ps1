...
function Wait-ForServices {
    ...
    while ($waited -lt $maxWait) {
        ...
        try {
            ...
            $healthyServices = docker-compose -f $ComposeFile ps --filter "status=running" --format "{{.Service}}" 2>$null
...
function Show-ServiceStatus {
    ...
    try {
        $services = docker-compose -f $ComposeFile ps --format "table {{.Name}}\t{{.Command}}\t{{.State}}\t{{.Ports}}"
        Write-Host $services
    } catch {
...
function Show-ApplicationURLs {
    ...
    Write-Host "`n🔧 Core Services:" -ForegroundColor White
