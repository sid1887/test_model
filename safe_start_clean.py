...
class CumpairStartup:
    ...
    def main(self, args) -> None:
        ...
        if not args.skip_preflight:
            if not self.run_pre_flight_check(quick_mode=args.quick_check):
                logger.error("Pre-flight check failed!")
                sys.exit(1)
        else:
            logger.warning("Skipping pre-flight check (not recommended)")
            
        # Step 3: Show startup info and start application
        self.show_startup_info()
        self.start_application(development_mode=args.dev)
...
def main():
    ...
    parser.add_argument("--skip-preflight", action="store_true", 
                       help="Skip pre-flight dependency check")    parser.add_argument("--dev", action="store_true",
